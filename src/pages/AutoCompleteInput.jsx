...
export default function AutoCompleteInput({
  ...
  useEffect(() => {
    setFilteredItems((prev) =>
      value
        ? prev.filter((item) => item.label.includes(value.toLowerCase()))
        : items
    );
  }, [value, items]);
  ...
  return (
    <div className="relative">
      <input
        type="text"
        placeholder=" "
        className={`peer px-3 py-2 border-2 rounded-md w-full font-medium ${
          error ? "border-red-600" : "border-sky-700"
        }`}
        onKeyDown={onKeyDown}
        autoComplete="off"
        {...registers}
